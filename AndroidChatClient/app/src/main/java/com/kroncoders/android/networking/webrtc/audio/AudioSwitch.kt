...
class AudioSwitch internal constructor(
    ...
    private val mutableAudioDevices = mutableListOf<AudioDevice>()
    ...
    fun activate() {
        ...
        when (state) {
            ...
            State.ACTIVATED -> selectedDevice?.let { selectDevice(it) }
    ...
    private fun hasNoDuplicates(preferredDeviceList: List<Class<out AudioDevice>>): Boolean {
        return preferredDeviceList.groupingBy { it }.eachCount().filter { it.value > 1 }.isEmpty()
    }
    ...
    private fun userSelectedDevicePresent(audioDevices: List<AudioDevice>) = userSelectedDevice?.let { selectedDevice ->
